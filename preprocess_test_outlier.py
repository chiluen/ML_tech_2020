...
test_d = test_d.drop(['agent','company'],axis=1)
test_d = test_d.dropna(axis = 0)
'''

#Deal with Outliers
test_d.loc[test_d.lead_time > 500,'lead_time'] = 500
test_d.loc[test_d.days_in_waiting_list > 0,'days_in_waiting_list'] = 1
test_d.loc[test_d.stays_in_weekend_nights >=  5,'stays_in_weekend_nights'] = 5
test_d.loc[test_d.adults > 4,'adults'] = 4
test_d.loc[test_d.previous_bookings_not_canceled > 0,'previous_bookings_not_canceled'] = 1
test_d.loc[test_d.previous_cancellations > 0,'previous_cancellations'] = 1
test_d.loc[test_d.stays_in_week_nights > 10,'stays_in_week_nights'] = 10
test_d.loc[test_d.booking_changes > 5,'booking_changes']=  5
test_d.loc[test_d.babies > 8,'babies'] = 0
test_d.loc[test_d.required_car_parking_spaces > 5,'required_car_parking_spaces'] = 0
test_d.loc[test_d.children > 8,'children']  = 0
#combine children and babies together as kids
test_d['kids'] = test_d.children + test_d.babies
#combine total mumbers by adding kids and adults
test_d['total_members'] = test_d.kids + test_d.adults
...
for i in range(test_d.shape[0]):
    ...
    if test_d["country"][i] not in countries:
        test_d['country'][i] = 'Others'
...
test_d = test_d.join(pd.get_dummies(test_d.country, prefix = 'country'))
